...
const snake = {
    ...
    getNextStepHeadPoint() {
        ...
        switch (this.direction) {
            case 'up':
                nextHeadPoint = {x: firstPoint.x, y: firstPoint.y - 1};
                break;
            case 'right':
                nextHeadPoint = {x: firstPoint.x + 1, y: firstPoint.y};
                break;
            case 'down':
                nextHeadPoint = {x: firstPoint.x, y: firstPoint.y + 1};
                break;
        ...
        nextHeadPoint = this.roundStepPoint(nextHeadPoint);
        return nextHeadPoint;
    },

    /**
     * Принимает точку и если она не помешается на доске (выходит за её пределы),
     * помешает её в игровое поле.
     * Например, следующая после правой границы точка будет противоположная — левая.
     * @param {{x: int, y: int}} point — точка-координана
     * @return {*}
     */
